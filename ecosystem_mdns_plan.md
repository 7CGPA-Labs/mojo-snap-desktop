...
## 3. Dual TCP/UDP Architecture and Low-Latency Binary Protocol

Sending text-based JSON over WebSockets introduces unnecessary overhead and parsing latency. All 3 platforms implement a dual TCP/UDP architecture (UDP for ultra-fast local LAN inputs, TCP WebSocket for WAN fallback) using the following ultra-fast binary payload for input syncing:
